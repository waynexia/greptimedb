...
pub enum Error {
    ...
    #[snafu(display("File size {size} is too small"))]
    FileSizeTooSmall {
        size: u64,
        #[snafu(implicit)]
        location: Location,
    ...
    #[snafu(display(
        "Unexpected meta size: max meta size {max_meta_size}, actual meta size {actual_meta_size}"
    ))]
    UnexpectedMetaSize {
        max_meta_size: u64,
    ...
    #[snafu(display("Invalid intermediate magic: {invalid:?}"))]
...
impl ErrorExt for Error {
    fn status_code(&self) -> StatusCode {
        ...
        match self {
            ...
            | DecodeProto { .. }
            | InvalidIntermediateMagic { .. }
            | Intermediate { .. } => StatusCode::Unexpected,
